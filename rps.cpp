 ...
 * Winter 2025
 *
 * a rock paper scissors game with two ways to play
 */
...
bool isRoundWinner(char move, char opponentMove) {
    ...
    if (opponentMove == 'R' || opponentMove == 'P' || opponentMove == 'S') {
...
int doRound(string p1Name, string p2Name) {
    ...
    bool p1Score = isRoundWinner(p1Move, p2Move);
    bool p2Score = isRoundWinner(p2Move, p1Move);
...
void rps() {
    ...
    while (userChoice != 3) {
        string winner = doGame(p1Name, p2Name, userChoice);
